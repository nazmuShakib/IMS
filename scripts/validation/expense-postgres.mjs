...
const directory = mkdtempSync(join(tmpdir(), 'ims-expense-pg-'));
...
try {
  run(join(bin, 'pg_ctl'), [
    ...
    '-o',
    `-p 55440 -k ${directory} -h 127.0.0.1`,
    'start',
  ]);
  started = true;
  run('npm', ['test', '--', 'tests/expense-postgres.test.ts'], {
    env: {
      ...process.env,
      TEST_EXPENSE_DATABASE_URL: `postgresql://${userInfo().username}@127.0.0.1:55440/postgres`,
    },
  });
} finally {
