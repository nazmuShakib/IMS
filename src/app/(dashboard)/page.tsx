...
import { DashboardMovers, DashboardNextInstallments, DashboardRecentActivity } from '@/components/dashboard/DashboardPeriodPanels';
...
import { getDashboard, getDashboardEmiSnapshot } from '@/services/dashboard';
...
export default async function DashboardPage() {
  ...
  const canViewEmi = hasPermission(role, 'VIEW_EMI');
  if (canViewEmi) await refreshEmiStatuses();
  const [dashboard, emiSnapshot] = await Promise.all([
    getDashboard(role),
    canViewEmi ? getDashboardEmiSnapshot() : Promise.resolve(null),
  ]);
  ...
  return (
    <>
      ...
      {role !== 'STAFF' && emiSnapshot && <div className="mb-4 grid gap-3 sm:grid-cols-3">
        <Link href="/emi">
          <Card className="h-full border-t-[3px] border-t-metric-revenue p-0">
            <div className="flex h-full flex-col px-3 py-2">
              ...
              <div className="mt-1 flex flex-1 flex-col justify-center rounded-[2px] bg-metric-revenue-wash px-2 py-1">
                <p className="text-[20px] font-semibold text-metric-revenue">{emiSnapshot.openContractCount}</p>
          ...
          <Card className="h-full border-t-[3px] border-t-metric-low p-0">
            <div className="flex h-full flex-col px-3 py-2">
              ...
              <div className="mt-1 flex flex-1 flex-col justify-center rounded-[2px] bg-metric-low-wash px-2 py-1">
                <p className="tnum text-[20px] font-semibold text-metric-low">{formatBDT(emiSnapshot.outstandingAmount)}</p>
        ...
        <Link href="/emi?status=OVERDUE">
          <Card className={`h-full border-t-[3px] p-0 ${emiSnapshot.overdueContractCount > 0 ? 'border-t-metric-profit-loss' : 'border-t-metric-neutral'}`}>
            <div className="flex h-full flex-col px-3 py-2">
              <p className="eyebrow">{t('emi.overdueContracts')}</p>
              <div className={`mt-1 flex flex-1 flex-col justify-center rounded-[2px] px-2 py-1 ${emiSnapshot.overdueContractCount > 0 ? 'bg-metric-profit-loss-wash' : 'bg-metric-neutral-wash'}`}>
                <p className={`text-[20px] font-semibold ${emiSnapshot.overdueContractCount > 0 ? 'text-metric-profit-loss' : 'text-metric-neutral'}`}>{emiSnapshot.overdueContractCount}</p>
                <p className={`text-[11px] ${emiSnapshot.overdueContractCount > 0 ? 'text-metric-profit-loss' : 'text-metric-neutral'}`}>{t('emi.overdueContractsHelp')}</p>
              </div>
            </div>
          </Card>
        </Link>
      ...
      {emiSnapshot && <DashboardNextInstallments installments={emiSnapshot.installmentsByPeriod} />}
