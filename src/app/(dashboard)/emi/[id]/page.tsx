...
export default async function EmiContractPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sale, customer, installments, payments] = await Promise.all([db.sales.findById(contract.saleId), db.customers.findById(contract.customerId), db.emi.findInstallments(id), db.emi.findPayments(id)]);
